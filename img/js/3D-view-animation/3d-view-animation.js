...
var m3D = function () {
      ...
      this.img.onclick = function () {
        ...
        if (this.diapo.isLoaded) {
          if (this.diapo.urlActive) {
            ...
            $('.feed-swiper').hide().removeClass('animate__animated animate__zoomInDown')
            /* ---- select current img ---- */
            cavasIndex = $(_that).index('canvas')
  ...
  Diapo.prototype.anim = function () {
    ...
    if (this.isLoaded) {
      ...
      /* ---- HTML rendering ---- */
      // 左侧偏移量
      this.css.left = Math.round(nw + 500 + x * p - w * .5) + 'px';

      this.css.top = Math.round(nh - 100 + y * p - h * .5) + 'px';
    ...
    if (camera.tx) {
      ...
      if (!m) {
        ...
        if (selected.url) {
          ...
          $('#screen canvas').show()
          $('.feed-swiper').eq(cavasIndex).show()
